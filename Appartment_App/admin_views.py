...
class view_approve_appartment(TemplateView):
    template_name = 'admin/appartment_view.html'
    def get_context_data(self, **kwargs):
        context = super(view_approve_appartment,self).get_context_data(**kwargs)
        view_image = add_appartment.objects.filter(status='notapproved')
        context['view_image'] = view_image
        return context

class view_appartments_accepted(View):
       def dispatch(self,request,*args,**kwargs):
        id = request.GET['id']

        view_image = add_appartment.objects.get(id=id)
        view_image.status = 'approved'
        view_image.save()
        return redirect(request.META['HTTP_REFERER'])

class view_appartments_reject(View):
       def dispatch(self,request,*args,**kwargs):
        id = request.GET['id']
        view_image = add_appartment.objects.get(id=id)
        view_image.status = 'reject'
        view_image.save()
        return redirect(request.META['HTTP_REFERER'])
