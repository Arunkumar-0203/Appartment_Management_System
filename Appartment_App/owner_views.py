...
class View_Booking(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        view_booking = book_now.objects.filter(owner_id_id=reg.id,payment='added',appartment__appartment_type='Flat')|book_now.objects.filter(owner_id_id=reg.id,payment='added',appartment__appartment_type='House')|book_now.objects.filter(owner_id_id=reg.id,payment='added',appartment__appartment_type='Hostel')
...
class View_pgBooking(TemplateView):
    template_name = 'owner/view_pgbooking.html'
    def get_context_data(self, **kwargs):
        context = super(View_pgBooking,self).get_context_data(**kwargs)
        boo= User.objects.get(id=self.request.user.id)
        reg=owner_reg.objects.get(user_id=boo.id)
        view_booking = book_now.objects.filter(owner_id_id=reg.id,payment='added',appartment__appartment_type='Paying Guest')
        context['view_booking'] = view_booking
        return context
    def post(self , request,*args,**kwargs):
        id1 = request.POST['id']
        action = request.POST['action']
        serviceamount = request.POST['serviceamount']
        rep = book_now.objects.get(id=id1)
        rep.action= action
        rep.status = 'replied'
        rep.amount = serviceamount
        rep.save()
        return redirect(request.META['HTTP_REFERER'])
...
class View_Services(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        R = service.objects.filter(user_id=id)
        context['service']=R
        return context

    def post(self,request,*args,**kwargs):
        id =self.request.user.id
        # app= add_appartment.objects.get(user_id=id)
        Name = request.POST['name']
        F = service()
        F.services = Name
        F.user_id=id
        # F.appartment_id=app.id
        F.save()
        return redirect(request.META['HTTP_REFERER'])
